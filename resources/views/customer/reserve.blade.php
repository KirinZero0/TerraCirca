    ...
    <body class="flex items-center justify-center min-h-screen bg-cover bg-center" style="background-image: url('{{ asset('assets/images/bg/bg.jpg') }}');">
        <div class="flex flex-col items-center">
            ...
            <div class="bg-white rounded-lg shadow-lg p-12 w-96">
                ...
                <form action="{{ route('customer.book') }}" enctype="multipart/form-data" method="post" novalidate onkeydown="return event.key !== 'Enter';">
                    @csrf
                    ...
                    <div class="mb-6">
                        <label for="date" class="block text-sm font-medium text-gray-700">Tanggal dan Waktu</label>
                        <input required  type="datetime-local" id="date" name="date" class="mt-1 p-3 border w-full">
                    </div>
                    <button type="submit" class="w-full bg-[#4693B1] rounded-md text-white py-3 hover:bg-[#596978]" onclick="return validateForm()">Book</button>
                </form>
